...
class Gaussian(nn.Module):
    def __init__(self, dim, size, sigma, act_layer, feature_extra=True):
        ...
        # [DDP 修复] 使用 register_buffer 确保在多卡训练时同步且不作为参数更新
...
class LFE_Module(nn.Module):
    def __init__(self, dim, stage, mlp_ratio, drop_path, act_layer, 
        ...
        # >>>>> 修改点：真正可选的 Scharr 和 Gaussian (设为 None 而不是替换为 Conv) <<<<<
        if stage == 0:
            if use_scharr:
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.stage == 0:
            ...
            if self.edge_extractor is not None:
                ...
                # [True Ablation] No Scharr 模式：直接透传，啥也不加
                x_att = x 
        ...
        # Stage 1-3: 处理高斯 (Gaussian)
            ...
            if self.gaussian is not None:
                ...
                # [True Ablation] No LFEA 模式：直接相加 (Residual Add)
                else:
                    x_att = x + att 
            else:
                # [True Ablation] No Gaussian 模式：没有高斯特征，自然也就没有 LFEA
                x_att = x 
...
class Stem(nn.Module):
    def __init__(self, in_chans, stem_dim, act_layer, use_log=True):
        ...
        # [True Ablation] No LoG 模式：使用普通卷积替代（因为 Stem 层必须要降采样，不能直接删除）
        if self.use_log:
...
class LWEGNet(nn.Module):
    ...
    def _freeze_stages(self, trainable_layers):
        ...
        # 冻结 Stages (映射关系: layer1->stage0, layer2->stage1, ...)
        # 这里逻辑是：如果 trainable_layers=3，则 layer3, layer4 可训练，即 stage2, stage3 可训练
        # 对应的索引是 index >= (4 - trainable_layers) ... 这种转换比较复杂，采用简单策略：
        # 根据 ResNet 习惯：trainable_layers=3 通常意味着 layer1(stage0) 冻结
        
        # 简单实现：冻结前 N 个 Stage
        # 例如 trainable_layers=3 (训练 3,4,5)，意味着冻结 stem, stage0, stage1 (共5层：stem, s0, s1, s2, s3)
        layers_to_freeze = 5 - trainable_layers # 5-3 = 2 -> freeze stem, s0
        
        if layers_to_freeze > 0:
            # Stem 已经在上面冻结了
            # 冻结后续 Stage
            for i in range(layers_to_freeze - 1): # -1 是因为 Stem 算一层
    ...
    def forward(self, x):
        x = self.Stem(x)
        outs = OrderedDict()
        # 映射关系: index 0->Stage1 output, 2->Stage2 output... (因为中间夹着 DRFD)
        # stages 结构: [Stage0, DRFD, Stage1, DRFD, Stage2, DRFD, Stage3]
        # output indices: 0, 2, 4, 6
...
def legnet_fpn_backbone(pretrain_path="", ablation_mode="full", trainable_layers=3, **kwargs):
    ...
    Args:
        ...
        ablation_mode (str): 消融实验模式 ('full', 'no_scharr', 'no_gaussian', 'no_lfea', 'no_log', 'baseline')
        trainable_layers (int): 可训练的层数，用于迁移学习冻结
    """
    # 默认全部开启
    use_log = True
    use_scharr = True
    ...
    # 动态计算 Channels (防止改 stem_dim 后报错)
    # Layer0: stem_dim * 2^0 = 32
    # Layer1: stem_dim * 2^1 = 64 ...
